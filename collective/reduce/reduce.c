...
int main(int argc, char ** argv) 
{
  ...
  int reduced_data[ARRAY_SIZE];
  int i;
  for (i=0;i<ARRAY_SIZE;i++)
    reduced_data[i] = 0;

  MPI_Reduce(data,reduced_data,ARRAY_SIZE,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);

  if (mype==0) printf("\nResult of MPI_Reduce w/ MPI_SUM:\n");
  print_process_data(reduced_data,mype,nprocs);
  
  MPI_Allreduce(data,reduced_data,ARRAY_SIZE,MPI_INT,MPI_SUM,MPI_COMM_WORLD);

  if (mype==0) printf("\nResult of MPI_Allreduce w/ MPI_SUM:\n");
  print_process_data(reduced_data,mype,nprocs);
  
  MPI_Allreduce(data,reduced_data,ARRAY_SIZE,MPI_INT,MPI_MAX,MPI_COMM_WORLD);

  if (mype==0) printf("\nResult of MPI_Allreduce w/ MPI_MAX:\n");
  print_process_data(reduced_data,mype,nprocs);
  
  MPI_Allreduce(data,reduced_data,ARRAY_SIZE,MPI_INT,MPI_LAND,MPI_COMM_WORLD);
